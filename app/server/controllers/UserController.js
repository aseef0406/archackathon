...
var maxTeamSize = process.env.TEAM_MAX_SIZE || 2;
...
UserController.getPage = function(query, callback){
  ...
  if (searchText.length > 0){
    ...
    queries.push({ 'profile.ideatitle': re });
...
UserController.updateIdeaById = function (id, idea, callback){
    User.findOneAndUpdate({
      _id: id,
      verified: true
    },
 ...
 * @param  {Function} callback      Callback with args (err, user)
 */
UserController.updateConfirmationById = function (id, idea, callback){
 ...
 */
UserController.getTeammates = function(id,teamname, callback){
  console.log("In getTeammates()"+teamname);
  User.findById(id).exec(function(err, user){
    if (err || !user){
      return callback(err, user);
    }

    var code = user.teamName;
...
UserController.getIdeas = function(id, callback){
...
UserController.getEmails = function(id, callback){
...
UserController.setTeamName = function(id, parameters, callback){
  console.log(parameters);
  var email1=parameters.split("*&*")[0];
  var teamName=parameters.split("*&*")[1];
  console.log(email1+" "+teamName );
  User.find({
    'teamName': teamName
  })
  .select('email')
...
UserController.getUserByEmail = function(id,email ,callback){
  console.log("In getEmails() by user"+email);
    User
      .find({
        email:email
      })
